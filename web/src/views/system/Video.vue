<template>
  <v-card>
    <v-toolbar flat height="64px">
      <v-toolbar-title>{{ $t("App.videoManager") }}</v-toolbar-title>
    </v-toolbar>
    <v-card-title>
    ...
    >
      <template v-slot:items="props">
        ...
        <td class="text-xs-center">{{ props.item.describe }}</td>
...
export default {
  ...
  computed: {
    headers() {
      return [
        ...
        { text: this.$t("App.videoName"), value: "id", align: "center" },
        { text: this.$t("App.videoLink"), value: "name", align: "center" },
        { text: this.$t("App.videoDescribe"), value: "describe", align: "center" },
        {
          text: this.$t("Sys.createTime"),
          value: "create_time",
          align: "center"
        },
        { text: this.$t("App.videoStatus"), value: "link", align: "center" },
  ...
  methods: {
    init() {
      axios.all([this.$api.video.get(), this.$api.role.get()]).then(
    ...
    setState(props) {
      ...
      this.$api.video.setState(props.item.id, status).then(() => {
        this.$notify.success("edit success", { timeout: 500 });
      });
    },
    editUser(props) {
      this.$api.video.getVideoById(props.item.id).then(data => {
        this.$data.Video = data.data;
        this.$data.dialog = true;
      });
    },
    resetPwd(props) {
      this.$api.video.resetPwd(props.item.id).then(() => {
        this.$notify.success("reset password success", { timeout: 500 });
      });
    },
    createOrUpdateVideo() {
      if (this.$data.isCreate) {
        let userInfo = Object.assign({}, this.$data.Video, this.$data.addUser);
        this.$api.video.createVideo(userInfo).then(() => {
          this.$data.isCreate = false;
          this.$api.video.get().then(data => {
        ...
        this.$api.video
          .editVideo(this.$data.Video.id, this.$data.Video)
          .then(() => {
    ...
    createUser() {
      ...
      let form = ["name", "link"];
      Object.keys(this.$data.Video).forEach(f => {
