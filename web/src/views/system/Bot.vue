<template>
  <v-card>
    <v-toolbar flat height="64px">
      <v-toolbar-title>{{ $t("App.botManager") }}</v-toolbar-title>
    </v-toolbar>

    <v-card-title>
      <v-btn color="primary" @click="createBot">{{
        $t("App.createBot")
      }}</v-btn>
      <v-spacer></v-spacer>
      <v-text-field
    ...
    >
      <template v-slot:items="props">
        ...
        <td class="text-xs-center">{{ props.item.property_key }}</td>
...
export default {
  ...
  computed: {
    headers() {
      return [
        ...
        { text: this.$t("App.botName"), value: "name", align: "center" },
        { text: this.$t("App.botUsername"), value: "username", align: "center" },
        { text: this.$t("App.botKey"), value: "key", align: "center" },
        {
          text: this.$t("Sys.createTime"),
          value: "create_time",
          align: "center"
        },
        { text: this.$t("App.botStatus"), value: "status", align: "center" },
  ...
  methods: {
    init() {
      axios.all([this.$api.bot.get(), this.$api.role.get()]).then(
