...
class AdvertiseController
{
    ...
    /**
     * 获取所有用户信息
     * @RequestMapping(route="/advertise", method={RequestMethod::GET}, params={"3", "获取所有广告"})
     * @OA\Get(
     *     path="/advertise",
     *     tags={"Advertise"},
     *     summary="获取所有广告信息",
     ...
     * )
     */
    public function getAdvertise(): array
    {
        $Advertises = $this->advertiseData->getAdvertises();

        return ReturnHelper::format('success', 200, $Advertises);
    }

    /**
     * 获取指定用户信息
     * @RequestMapping(route="{id}", method={RequestMethod::GET}, params={"3", "获取指定用户信息"})
     * @OA\Get(
     *     path="/advertise/{id}",
     *     tags={"Advertise"},
     *     summary="获取指定用户信息",
    ...
    public function getUser(int $id)
    {
        ...
        $advertises = $this->advertiseData->getAdvertise($id);
    ...
    /**
     * 创建广告
     * @RequestMapping(route="/advertise", method={RequestMethod::POST}, params={"3", "创建广告"})
     * @Validate(validator="AdvertiseValidator", params={"nickname", "email","phone", "identity", "credential"})
     * @OA\Post(
     *     path="/user",
     *     tags={"User"},
     *     summary="创建用户",
    ...
    {
        $uadvertiseInfo = $request->post();
        $clientInfo = $request->getCoRequest()->server;
        $token      = $request->getHeader('Authorization');
        $token      = explode(' ', $token[0])[1];

        $ret = $this->advertiseData->createAdvertise($uadvertiseInfo);
    ...
    /**
     * 修改用户信息
     * @RequestMapping(route="{id}", method={RequestMethod::PUT}, params={"3", "修改广告"})
     * @Validate(validator="AdvertiseValidator", fields={"name", "link"})
     * @OA\Put(
     *     path="/advertise/{id}",
     *     tags={"Advertise"},
     *     summary="修改广告",
     ...
     * @throws ApiException
     */
    public function editAdvertise(Request $request, int $id)
    {
        $fields   = [
            "name",
            "link",
        ...
        foreach ($fields as $field) {
            $advertiseInfo[$field] = $request->post($field);
        }

        $advertiseInfo['update_time'] = date('Y-m-d H:i:s');


        $ret = $this->advertiseData->updateAdvertise($id, $advertiseInfo);
        if ($ret) {
            return ReturnHelper::format('success', 200, $ret);
        } else {
     ...
     * @Validate(validator="AdvertiseValidator", fields={"status"}, type=ValidateType::GET)
    ...
    public function setStatus(Request $request, int $id)
    {
        ...
        $ret = $this->advertiseData->updateAdvertise($id, $status);
